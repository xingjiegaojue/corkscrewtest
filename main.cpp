...
int32 unwind_backtrace_signal(void* sigcontext, uint64* Backtrace, int32 MaxDepth)
{
    ...
    backtrace_frame_t* BackTraceFrames = static_cast<backtrace_frame_t*>(malloc(BackTraceFramesSize));
    memset(BackTraceFrames, 0,BackTraceFramesSize);

    NumFrames = unwind_backtrace_signal_arch(sigcontext, BackTraceFrames, 0, MaxDepth);
    qDebug() << "unwind_backtrace_signal:: " << NumFrames;
...
// Use the followings to print crash stack
#define MAX_BACKTRACE_LINE_LENGTH 1024

static struct sigaction oldAct;

static void registerSignalHandle(struct sigaction *act, struct sigaction *oact) {
...
// Print crash stack
static void _handleNativeCrash(int reason, struct siginfo *siginfo, void *sigcontext) {
    uint64 backTraceFms[MAX_BACKTRACE_LINE_LENGTH] = { 0 };

    unwind_backtrace_signal(sigcontext,backTraceFms,MAX_BACKTRACE_LINE_LENGTH);

    oldAct.sa_sigaction(reason,siginfo,sigcontext);
...
static void registerCrashHandle() {
    qDebug() << "registerCrashHandle";

    struct sigaction act;
    sigemptyset(&act.sa_mask);             //清空此信号集
    act.sa_flags = SA_SIGINFO;
    act.sa_sigaction = _handleNativeCrash;
    registerSignalHandle(&act, &oldAct);
}
...
static void _handleNativeCrash(int reason, struct siginfo *siginfo, void *sigcontext) {
    ...
    void * libcorkscrew = dlopen("libcorkscrew.so", RTLD_NOW|RTLD_GLOBAL);
    if (libcorkscrew == NULL) {
        return;
    }

    t_unwind_backtrace unwind_backtrace = (t_unwind_backtrace) dlsym(
            libcorkscrew, "unwind_backtrace");
    t_unwind_backtrace_signal_arch unwind_backtrace_signal_arch = (t_unwind_backtrace_signal_arch) dlsym(
            libcorkscrew, "unwind_backtrace_signal_arch");
    t_acquire_my_map_info_list acquire_my_map_info_list = (t_acquire_my_map_info_list) dlsym(
            libcorkscrew, "acquire_my_map_info_list");
    t_release_my_map_info_list release_my_map_info_list = (t_release_my_map_info_list) dlsym(
            libcorkscrew, "release_my_map_info_list");
    t_get_backtrace_symbols get_backtrace_symbols = (t_get_backtrace_symbols) dlsym(libcorkscrew,
            "get_backtrace_symbols");
    t_free_backtrace_symbols free_backtrace_symbols = (t_free_backtrace_symbols) dlsym(libcorkscrew,
    ...
    //qDebug() << "reason" << reason << "size: " << size;
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (method) {
            uintptr_t pc_offset = symbols[i].relative_pc - symbols[i].relative_symbol_addr;
            if (pc_offset) {
                snprintf(line, MAX_BACKTRACE_LINE_LENGTH,
                        "#%02d  pc %08x %08x  %.*s (%.*s+%u)", i,
                        symbols[i].relative_pc,symbols[i].relative_symbol_addr, fieldWidth, file,
                        fieldWidth, method, pc_offset);
            } else {
                snprintf(line, MAX_BACKTRACE_LINE_LENGTH,
                        "#%02d  pc %08x %08x  %.*s (%.*s)", i,
                        symbols[i].relative_pc,symbols[i].relative_symbol_addr, fieldWidth, file,
                        fieldWidth, method);
            }
        } else {
...
int main(int argc, char *argv[])
{
    ...
    if (stack.ss_sp != NULL && sigaltstack(&stack, NULL) == 0) {
        qDebug() << "init signal callback stack size failed.";
    }
